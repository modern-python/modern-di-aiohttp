...
| Home | Holds |
|---|---|
| `modern_di_aiohttp/` | anything readable from the module — the default |
...
> Can an agent get this by reading `modern_di_aiohttp/`? → **don't write it.**
...
The long-form user guide is not in this repo: it is
[`docs/integrations/aiohttp.md`](https://github.com/modern-python/modern-di/blob/main/docs/integrations/aiohttp.md)
in `modern-di`. A user-visible behaviour change is two PRs, and the one here is not done until the
other is open.
